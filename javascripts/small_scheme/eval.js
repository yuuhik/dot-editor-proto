...
LEval.prototype._lEval = function (s, env) {
  ...
  if (s instanceof LCell) {
    ...
    if (head instanceof LClosure) {
      // console.log('head',head, head instanceof LClosure);
      return this.lApply(head, // 関数部
                         cdr(s), // 残りの引数
                         env);
    }
    if (head instanceof LPrimitiveFunc) {
      // console.log('head',head, head instanceof LPrimitiveFunc);
      return this.lApply(head, // 関数部
                         cdr(s), // 残りの引数
                         env);
    }
...
LEval.prototype.lookupVariableValue = function(variable, environment) {
  // console.log(variable,'lookupVariableValue environment',environment);

  for (var i = environment.length - 1; i >= 0; --i) {
    var frame = environment[i];
    // console.log(variable, frame);
    // console.log(frame.binds[variable]);
...
LEval.PrimitiveFunctions = (function () {
  ...
  console.log(Board.scheme_primitive_functions);
  result = Board.scheme_primitive_functions.install_JS_functions(result);
